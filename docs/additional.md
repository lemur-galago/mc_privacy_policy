...
> [!IMPORTANT]
> Все примеры ниже рассчитаны на работы с платформой версии 6.1 и выше. При использовании мобильного клиента со 
> старшими версиями платформы вместо реализации абстрактного действия `onWebClientInit[]` в реализацию абстрактного 
> действия `onWebClientStarted[]` следует добавить строку: `INTERNAL CLIENT 'mdt.js';`
...
обязательных параметра `tag` и `barcode`. При вызове callback-функции параметре `tag` будет передаваться значение, 
...
Если параметр `callback` опущен, то, если хотя бы один штрихкод прочитан и успешешно распознан, то вместо обратного 
...
`tag` и `barcode`. При вызове callback-функции параметре `tag` будет передаваться значение, которое было передано в 
...
заголовке запроса будут переданы параметры Basic-авторизации с логином и паролем пользователя, авторизированного в 
приложении.
